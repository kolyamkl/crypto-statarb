...
def stats_row(name: str, s: dict[str, float], bar_unit: str = "h") -> str:
    cells = [
        ...
        fmt(s.get("avg_hold_bars"), ".0f") + bar_unit if "avg_hold_bars" in s else "—",
...
def equity_drawdown_plot(
    series: dict[str, pd.Series], title: str, fname: str, cfg: Config
) -> Path:
...
def main() -> None:
    ...
    bar_unit = "d" if interval == "1d" else "h"

    with connect(cfg.db) as conn:
        # Universe + aux so benchmark columns (e.g. SPY) exist; the screen never sees aux.
    ...
    params, _, _, _, port, results = static_split(panel, funding, cfg, open_panel)
    ...
    lines = [
        ...
        "",
        header, divider,
        *[stats_row(name, s, bar_unit) for name, s in rows],
        "",
        "## Per-pair, test window",
        "",
        header, divider,
        *[stats_row(name, s, bar_unit) for name, s in pair_rows],
    ]
    out = report_path(cfg, "m6_metrics.md")
    ...
    log.info("plot: %s", equity_drawdown_plot(
        ...
        "frozen config over full history (train + test)", "strategy_full.png", cfg,
    ))
